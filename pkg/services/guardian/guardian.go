...
func (g *dashboardGuardianImpl) logHasPermissionResult(permission m.PermissionType, hasPermission bool, err error) (bool, error) {
	...
	if hasPermission {
		g.log.Debug("User granted access to execute action", "orgId", g.orgId, "folderId/dashboardId", g.dashId, "action", permission)
	} else {
		g.log.Debug("User denied access to execute action", "orgId", g.orgId, "folderId/dashboardId", g.dashId, "action", permission)
	}
